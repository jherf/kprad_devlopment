...
from main.util.physics import log_lambda_ei, eta_parallel
...
def fkprad(t: float, solV: np.ndarray, injectors: list, params: dict) -> np.ndarray:
    ...
    Tfac = np.sqrt(
        Te / Ti
    )  # scaling factor for optical depth (assuming Doppler broadening dominates)
    ...
    for sym in layout.elements:
        ...
        tau = (
            Tfac * dens[sym] * Rmin * 100.0
        )  # column density of this charge state [/cm^2] (Zmax+1, Nr)
        ...
        dUdt += np.einsum("zr,z->r", dn[1 : Zmax + 1, :], rateStruct.Ei_cumulative[sym])
