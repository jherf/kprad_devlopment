...
import numpy as np
from kprad.util.constants import _MIN_TE
...
def log_lambda_ei(ne, Te, Zeff):
    ...
    if loc.any():
        val = np.clip(
            np.sqrt(ne[loc]) * Zeff[loc] * Te[loc] ** (-1.5), 1e-9, None
        )
...
def beta_e(Zeff):
    ...
    if False:
        ...
        ax.plot(z_new, beta_e_model(z_new, 0.582, 0.94, 1.25), color = 'tab:blue', label = "Old Fit")
        ax.set_ylabel('Zeff')
...
def beta_T(ne, Te, Zeff, Rmaj, Rmin, q=2.0):
    ...
    LL = log_lambda_ei(ne,Te,Zeff,)
    tau_e = 3.45e5*Te**1.5/(ne*LL) #  % electron-electron collision time [s]
    eps = Rmin/Rmaj # inverse aspect ratio
    fT = 1 - (1-eps)**2/(np.sqrt(1-eps**2)*(1+1.46*np.sqrt(eps))) # trapping fraction
    vbar_e = 4.19e7*np.sqrt(Te) #electron thermal velocity [cm/s]
    nustar_e = 100*Rmaj*q/(vbar_e*tau_e*eps**1.5) # electron collision rate normalized to orbit frequency
